...
<head>
    ...
    <script src="https://www.gstatic.com/firebasejs/11.0.1/firebase-vertexai-compat.js"></script>
    ...
    <style>
        * {
        ...
        .still-searching {
            background: linear-gradient(90deg, rgba(0, 102, 255, 0.1), rgba(0, 170, 255, 0.1));
            border: 1px solid rgba(0, 102, 255, 0.3);
            border-radius: 8px;
...
<body>
    <div class="container">
        <header>
            ...
            <span class="version-badge">v1.9.7</span>
        </header>
        ...
        <div class="existing-profile-card" id="existingProfileCard" style="display: none;">
            <div class="existing-profile-header">
                ...
                <div class="existing-profile-info">
                    <span class="database-badge">✓ In LitLyric Database</span>
        ...
        <div class="result-card" id="resultCard">
            <div class="still-searching" id="stillSearching" style="display: none;">
                <span class="searching-spinner"></span> Still searching for more sources...
            </div>
    ...
    <script>
        ...
        async function searchWikipedia(name) {
            ...
            for (const variant of uniqueVariants) {
                ...
                try {
                    ...
                    log(`Wikipedia: Found! "${data.title}" - Bio: ${result.bio?.length || 0} chars, Images: ${result.images.length}`);
                    return result;
            ...
            try {
                ...
                if (!bioFound) {
                    ...
                    while ((match = paragraphRegex.exec(html)) !== null) {
                        ...
                        if (text.includes('»') || text.includes('BESTSELLER') || 
                        ...
                        for (const aboutPath of aboutPages) {
                            try {
                                ...
                                if (aboutHtml.length < 1000 || 
                                    ...
                                    log(`Personal Site: ${aboutPath} appears to be a 404 page, skipping`);
                                    continue;
                                }
                                ...
                                const paragraphMatches = aboutHtml.matchAll(/<p[^>]*>([\s\S]*?)<\/p>/gi);
                                const paragraphs = [];
                                for (const match of paragraphMatches) {
                                    ...
                                    if (text.length > 50 && 
                                        ...
                                        !text.toLowerCase().includes('page you were looking for') &&
                                        !text.toLowerCase().includes("couldn't find")) {
                                        paragraphs.push(text);
                                        log(`Personal Site: Found paragraph (${text.length} chars): "${text.substring(0, 50)}..."`);
                                    }
                                }
                                ...
                                if (paragraphs.length > 0) {
                                    ...
                                    log(`Personal Site: No valid paragraphs found on ${aboutPath}`);
                                }
                            } catch (e) {
        ...
        async function searchIMDB(name) {
            ...
            try {
                ...
                const suggestionUrl = `https://v2.sg.media-imdb.com/suggestion/${firstLetter}/${encodeURIComponent(name.toLowerCase().replace(/\s+/g, '_'))}.json`;
                ...
                try {
                    ...
                    if (data.d && data.d.length > 0) {
                        ...
                        for (const person of data.d) {
                            ...
                            if (personNameLower === searchNameLower || 
                        ...
                        if (bestMatch) {
                            ...
                            if (lastNameFound === lastNameSearch) {
                                ...
                                log(`IMDB: Found "${bestMatch.l}" via suggestion API (${bestMatch.id})`);
                                
                                // Save suggestion image as fallback
                                if (bestMatch.i && bestMatch.i.imageUrl) {
                                    suggestionImage = bestMatch.i.imageUrl.replace(/U[XY]\d+/g, 'UX400');
                                }
                            } else {
                                log(`IMDB: Name mismatch - searched "${name}" but found "${bestMatch.l}"`, 'warning');
                            }
                        }
                    }
                } catch (e) {
                ...
                const cleanImdbBio = (bio, personName) => {
                    ...
                    const prefixPattern = /^[^.]+\.\s*(?:Actor|Actress|Director|Writer|Producer|Voice)[^.]*\.\s*/i;
                    cleaned = cleaned.replace(prefixPattern, '');
                    
                    // Also try removing just the name at the start
                    if (personName) {
                        const namePattern = new RegExp(`^${personName.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}[.,:]?\\s*`, 'i');
                ...
                const jsonLdMatch = profileHtml.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/i);
                if (jsonLdMatch) {
                    try {
                        const jsonData = JSON.parse(jsonLdMatch[1]);
                        if (jsonData.description && jsonData.description.length > 50) {
                            result.bio = cleanImdbBio(jsonData.description, result.name || name);
                            log(`IMDB: Found bio from JSON-LD (${result.bio.length} chars)`);
                        }
                    } catch (e) {
                ...
                if (!result.bio) {
                    try {
                        ...
                        const bioJsonMatch = bioHtml.match(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/i);
                        if (bioJsonMatch) {
                            try {
                                const jsonData = JSON.parse(bioJsonMatch[1]);
                                if (jsonData.description && jsonData.description.length > 50) {
                                    result.bio = cleanImdbBio(jsonData.description, result.name || name);
                                    log(`IMDB: Found bio from bio page JSON-LD (${result.bio.length} chars)`);
                                }
                            } catch (e) {}
                        }
                            ...
                            for (const pattern of bioPagePatterns) {
                                ...
                                if (match && match[1]) {
                                    ...
                                    if (bio.length > 50) {
                                        ...
                                        log(`IMDB: Found bio from bio page (${result.bio.length} chars)`);
                                        break;
                                    }
                                }
                            }
                ...
                for (const pattern of bioPatterns) {
                    ...
                    if (bioMatch && bioMatch[1]) {
                        ...
                        if (bioText.length > 50 && !bioText.includes('{') && !bioText.includes('function')) {
                ...
                for (const pattern of headshotPatterns) {
                    ...
                    for (const match of matches) {
                        ...
                        if (imgUrl.includes('CR22,0,90,133') || imgUrl.includes('_UX90_')) {
                            continue;
                        }
                ...
                if (result.images.length < maxImages) {
                    ...
                    if (hasPhotos) {
                        ...
                        try {
                            ...
                            const mediaPattern = /(https:\/\/m\.media-amazon\.com\/images\/M\/MV[^"]+\.jpg)/gi;
                            ...
                            if (result.images.length > 1) {
                                log(`IMDB: Found ${result.images.length} images total from profile + media pages`);
                            }
                        } catch (e) {
                ...
                if (result.images.length > 0 || result.bio) {
                    log(`IMDB: Returning ${result.images.length} images, bio: ${result.bio ? result.bio.length + ' chars' : 'none'}`);
                    return result;
                }
        ...
        async function performSearch(skipDatabaseCheck = false) {
            ...
            if (firebaseBtn) {
                ...
                firebaseBtn.innerHTML = '☁️ Submit to LitLyric Database';
            }
            ...
            try {
                ...
                const updateLiveDisplay = () => {
                    if (liveResults.length > 0) {
                        const merged = mergeResults(narratorName, liveResults.filter(r => r));
                ...
                const createSearch = (name, searchFn, timeout, statusKey, checkImage = false) => {
                    return (async () => {
                ...
                const coreSearches = [
                    createSearch('Audible', searchAudible(narratorName), 45000, 'audible'),
                    createSearch('Wikipedia', searchWikipedia(narratorName), 45000, 'wikipedia'),
                    createSearch('AudioFile', searchAudioFile(narratorName), 45000, 'audiofile'),
                ...
                const additionalSearches = fastMode ? [] : [
                    createSearch('Personal Site', searchPersonalSite(narratorName), 30000, 'personalsite'),
                    createSearch('Voices to Know', searchVoicesToKnow(narratorName), 30000, 'voicestoknow'),
                    createSearch('Soundbooth', searchSoundbooth(narratorName), 30000, 'soundbooth'),
                    createSearch('SoundCloud', searchSoundCloud(narratorName), 30000, 'soundcloud')
                ];
                ...
                if (!hasImage) {
                    ...
                    try {
                        const googleResult = await fetchWithTimeout(searchGoogle(narratorName), 30000);
                        updateSourceStatus('google', googleResult ? 'found' : 'not-found');
                        if (googleResult) {
                            liveResults.push(googleResult);
                        }
                    } catch (e) {
        ...
        async function displayResult(data) {
            ...
            document.getElementById('sourceBadges').innerHTML = data.sources.map(s =>
                `<span class="source-badge">${SOURCES[s.name]?.icon || '📄'} ${s.name}</span>`
            ).join('');
            ...
            if (data.images.length > 0 || true) {  // Always show section for custom input
                ...
                let imagesHtml = `
                    <div class="image-option ${data.selectedImage === null ? 'selected' : ''}" 
                        ...
                        <div class="image-option-check">✓</div>
                    </div>
                ...
                imagesHtml += data.images.map((img, i) => `
                    <div class="image-option ${i === data.selectedImage ? 'selected' : ''}" 
                        ...
                        <div class="image-option-info">
                            <div class="image-option-source">${SOURCES[img.source]?.icon || '📄'} ${img.source}</div>
            ...
            if (data.bios.length > 0 || true) {  // Always show section for custom input
                ...
                let biosHtml = `
                    <div class="bio-option ${data.selectedBio === null ? 'selected' : ''}" 
                        ...
                        <div class="bio-option-header">
                            <span class="bio-option-source">✕ None</span>
                            <div class="bio-option-check">${data.selectedBio === null ? '✓' : ''}</div>
                        </div>
                    ...
                    <div class="bio-option ${data.selectedBio === 'custom' ? 'selected' : ''}" 
                        ...
                        <div class="bio-option-header">
                            <span class="bio-option-source">＋ Custom</span>
                            <div class="bio-option-check">${data.selectedBio === 'custom' ? '✓' : ''}</div>
                        </div>
                ...
                biosHtml += data.bios.map((bio, i) => `
                    <div class="bio-option ${i === data.selectedBio ? 'selected' : ''}" 
                        ...
                        <div class="bio-option-header">
                            <span class="bio-option-source">${SOURCES[bio.source]?.icon || '📄'} ${bio.source}</span>
                            <span class="bio-option-length">${bio.length} chars</span>
                            <div class="bio-option-check">${i === data.selectedBio ? '✓' : ''}</div>
                        </div>
            ...
            document.getElementById('sourcesList').innerHTML = data.sources.map(s =>
                `<a href="${s.url}" target="_blank" class="source-link">${SOURCES[s.name]?.icon || '📄'} ${s.name}</a>`
            ).join('');
        ...
        async function uploadToFirebase() {
            ...
            try {
                ...
                btn.textContent = '✓ Uploaded to Database';
        ...
        async function generateAIBio() {
            ...
            try {
                const prompt = `"${currentData.name}" narrator bio approximately 4 sentences.`;
                ...
                setTimeout(() => {
                    btn.textContent = '✨ Generate Bio with AI';
                    btn.disabled = false;
